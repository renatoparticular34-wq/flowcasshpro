  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-500">Defina os parâmetros da sua empresa e o plano de contas.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-emerald-600">Contas de Entrada</h3>
              <span className="text-xs font-bold bg-emerald-50 text-emerald-600 px-2 py-1 rounded-md">RECEITAS</span>
            </div>
            <div className="space-y-3 flex-1">
              {incomeAccounts.map((acc, idx) => (
                <div key={acc.id} className="flex items-center gap-3">
                  <span className="text-slate-300 font-mono text-xs w-6">{String(idx + 1).padStart(2, '0')}</span>
                  <input
                    type="text"
                    value={acc.name}
                    placeholder="Nome da conta..."
                    onChange={(e) => handleAccountChange(acc.id, e.target.value)}
                    className="flex-1 px-4 py-2 bg-slate-50 border border-slate-100 rounded-lg text-sm font-medium focus:bg-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                  />
                  <button
                    type="button"
            ...
            <button
              ...
              className="mt-6 w-full py-2 border-2 border-dashed border-emerald-100 text-emerald-600 font-bold rounded-xl hover:bg-emerald-50 transition-all text-sm"
            >
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-rose-600">Contas de Saída</h3>
              <span className="text-xs font-bold bg-rose-50 text-rose-600 px-2 py-1 rounded-md">DESPESAS</span>
            </div>
            <div className="space-y-3 flex-1">
              {expenseAccounts.map((acc, idx) => (
                <div key={acc.id} className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    placeholder="Nome da conta..."
                    onChange={(e) => handleAccountChange(acc.id, e.target.value)}
                    className="flex-1 px-4 py-2 bg-slate-50 border border-slate-100 rounded-lg text-sm font-medium focus:bg-white focus:ring-2 focus:ring-rose-500 outline-none transition-all"
                  />
                  <button
                    type="button"
                    onClick={() => handleDeleteAccount(acc.id)}
            ...
            <button
              ...
              className="mt-6 w-full py-2 border-2 border-dashed border-rose-100 text-rose-600 font-bold rounded-xl hover:bg-rose-50 transition-all text-sm"
            >
        ...
        <div className="bg-amber-50 border border-amber-200 p-6 rounded-2xl flex gap-4">
          ...
          <div>
            <h4 className="font-bold text-amber-800">Atenção Estrutural</h4>
            <p className="text-sm text-amber-700">As contas alimentam automaticamente os menus de seleção nos lançamentos. Evite deixar campos vazios ou duplicar nomes para manter a integridade dos relatórios.</p>
          </div>
        </div>
