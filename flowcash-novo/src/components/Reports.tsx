  ...
  return (
    <div class="space-y-8">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 class="text-3xl font-extrabold text-slate-900">Fluxo de Caixa (FDC)</h1>
          <p class="text-slate-500">Consolidado mensal estruturado para 12 meses.</p>
        </div>

        <div class="flex gap-4 no-print sm:flex-wrap">
          <button
          ...
          <div class="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-slate-200">
            <button onClick={() => setViewYear(y => y - 1)} class="p-1 hover:bg-slate-100 rounded-lg">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <span class="font-bold text-slate-900 px-2">{viewYear}</span>
            <button onClick={() => setViewYear(y => y + 1)} class="p-1 hover:bg-slate-100 rounded-lg">
          ...
          <select
            ...
            class="px-4 py-2 bg-white border border-slate-200 rounded-xl font-bold text-slate-700 outline-none"
          >
        ...
        <div class="overflow-x-auto">
          <table class="w-full text-xs font-medium border-collapse">
            <thead class="bg-slate-900 text-white">
              <tr>
                <th class="p-3 text-left border border-slate-800 min-w-[200px]">Descrição / Mês</th>
                {months.map(m => (
                  <th key={m} class="p-3 text-center border border-slate-800 min-w-[100px]">{m}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              <tr class="bg-indigo-50/50">
                <td class="p-3 font-bold text-slate-900 border border-slate-100">Saldo Inicial</td>
                {consolidation.map((data, i) => (
                  <td key={i} class="p-3 text-right font-bold text-slate-900 border border-slate-100">
              ...
              {/* Incomes */}
              <tr class="bg-emerald-50 text-emerald-800 uppercase text-[9px] tracking-widest font-bold">
                <td colSpan={13} class="p-2 px-4 border border-slate-100">Receitas</td>
              </tr>
              {accounts.filter(a => a.type === AccountType.INCOME).map(acc => (
                <tr key={acc.id} class="hover:bg-slate-50">
                  <td class="p-3 pl-6 border border-slate-100 text-slate-600 font-medium">{acc.name}</td>
                  {consolidation.map((data, i) => (
                    <td key={i} class="p-3 text-right border border-slate-100 text-emerald-600">
                      {data.breakdown[acc.id] > 0 ? formatCurrency(data.breakdown[acc.id]) : '-'}
                    </td>
                  ))}
                </tr>
              ))}
              <tr class="bg-emerald-100/50 font-bold">
                <td class="p-3 border border-slate-100 text-emerald-800">Total de Entradas</td>
                {consolidation.map((data, i) => (
                  <td key={i} class="p-3 text-right border border-slate-100 text-emerald-800">
              ...
              {/* Expenses */}
              <tr class="bg-rose-50 text-rose-800 uppercase text-[9px] tracking-widest font-bold">
                <td colSpan={13} class="p-2 px-4 border border-slate-100">Despesas</td>
              </tr>
              {accounts.filter(a => a.type === AccountType.EXPENSE).map(acc => (
                <tr key={acc.id} class="hover:bg-slate-50">
                  <td class="p-3 pl-6 border border-slate-100 text-slate-600 font-medium">{acc.name}</td>
                  {consolidation.map((data, i) => (
                    <td key={i} class="p-3 text-right border border-slate-100 text-rose-600">
                      {data.breakdown[acc.id] > 0 ? formatCurrency(data.breakdown[acc.id]) : '-'}
                    </td>
                  ))}
                </tr>
              ))}
              <tr class="bg-rose-100/50 font-bold">
                <td class="p-3 border border-slate-100 text-rose-800">Total de Saídas</td>
                {consolidation.map((data, i) => (
                  <td key={i} class="p-3 text-right border border-slate-100 text-rose-800">
              ...
              {/* Bottom Line */}
              <tr class="bg-slate-50 font-extrabold border-t-2 border-slate-200">
                <td class="p-3 border border-slate-100 text-slate-900">Resultado Mensal</td>
                {consolidation.map((data, i) => (
              ...
              <tr class="bg-slate-900 text-white font-extrabold">
                <td class="p-4 border border-slate-800 text-sm">Saldo Acumulado</td>
                {consolidation.map((data, i) => (
                  <td key={i} class="p-4 text-right border border-slate-800 text-sm">
