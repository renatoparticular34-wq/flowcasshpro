...
interface TransactionsProps {
  ...
  onTempBalanceChange?: (value: number) => void;
  ...
  return (
    <div class="space-y-6">
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 class="text-3xl font-extrabold text-slate-900">Movimentos</h1>
          <p class="text-slate-500">Gerencie suas entradas e saídas diárias.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={onAdd}
            class="flex items-center gap-2 px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 no-print"
          >
      ...
      <div class="bg-amber-50 border border-amber-200 p-6 rounded-2xl">
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1">
            <h3 class="font-bold text-amber-800 mb-3">Saldo Inicial Temporário (Opcional)</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label class="block text-sm font-semibold text-amber-800 mb-2">Valor do Saldo</label>
                <div class="relative">
                  <span class="absolute left-4 top-1/2 -translate-y-1/2 text-amber-600 font-bold">R$</span>
                  <input
                    type="text"
                    value={tempInitialBalance ? formatCurrencyValue(tempInitialBalance) : ''}
                    onChange={handleCurrencyChange}
                    class="w-full pl-12 pr-4 py-3 bg-white border border-amber-300 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none font-medium"
                    placeholder="0,00"
                  />
                </div>
              </div>
              <div>
                <label class="block text-sm font-semibold text-amber-800 mb-2">Mês de Referência</label>
                <select
                  value={tempBalanceMonth}
                  onChange={(e) => onTempBalanceMonthChange(parseInt(e.target.value))}
                  class="w-full px-4 py-3 bg-white border border-amber-300 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none font-medium"
                >
                  {[
              ...
              <div>
                <label class="block text-sm font-semibold text-amber-800 mb-2">Ano de Referência</label>
                <select
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4">
        <div class="flex-1 relative">
          ...
          <input
            ...
            class="w-full pl-12 pr-4 py-3 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none"
          />
        </div>
        <div class="flex gap-4">
          <select
            ...
            class="px-4 py-3 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-medium text-slate-700"
          >
          ...
          <table class="w-full text-left">
            <thead class="bg-slate-50 border-b border-slate-100">
              <tr>
                ...
                <th class="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Conta</th>
                ...
                <tr key={t.id} className={`${t.status === TransactionStatus.PENDING ? 'bg-orange-50/50' : 'hover:bg-slate-50'} transition-colors group`}>
                  ...
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-sm font-semibold text-slate-900">{getAccountName(t.accountId)}</span>
                    </div>
                  </td>
